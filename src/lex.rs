...
impl<'b> From<&'b [u8]> for Lex<'b> {
  fn from(buf: &'b [u8]) -> Self {
    Self { buf, index: usize::default(), len: usize::default(), loc: Loc::default() }
  }
}
...
impl<'b> Iterator for Lex<'b> {
  ...
  fn next(&mut self) -> Option<Self::Item> {
    macro_rules! next {
      ($on_eof:block) => {
        if let Some(&c) = self.buf.get(self.index) {
          self.index += 1;
          self.len += 1;
          self.loc.inc(c);
          c
    ...
    macro_rules! tok {
      () => {{
        let tok = &self.buf[self.index..self.index + self.len];
        self.len = 0;
        tok
      }};
    }
    ...
    macro_rules! err {
      ($loc:expr, $err:expr) => {
        Some(Err(Err::new(srcloc!(), $loc, $err)))
      };
    }
